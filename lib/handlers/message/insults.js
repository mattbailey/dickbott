...
var cacheData = () => {
  var db = new sqlite3.Database('./lib/handlers/message/data/wordlist.sql')
...
var grepLexicon = query => {
  ...
  for (trigger of Object.keys(triggers)) {
    if (trigger.match(re)) { results += trigger + " " }
    if (triggers[trigger].match(re)) { results += triggers[trigger] + " "}
  }

  results += "*insults*: "
  for (insult of insults) {
    if (insult.match(re)) { results += insult + " " }
  }
...
module.exports = {
  ...
  handler: async (bot, args, msg) => {
    ...
    if (trigger === 'GREP') {
      ...
      console.log("grep query is "+query[1])
